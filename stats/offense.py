...
hits.loc[hits['inning']] = pd.to_numeric(hits.loc[hits['inning']])

replacements = {r'^S(.*)': 'single', r'^D(.*)': 'double', r'^T(.*)': 'triple', r'^HR(.*)': 'hr'}

hit_type = hits['event'].replace(replacements, regex=True)

hits = hits.assign(hit_type=hit_type)  # Assigning new column groupings to hits dataframe

strike_outs = strike_outs.groupby(['year','game_id']).size()

hits = hits.groupby(['inning','hit_type']).size().reset_index()
